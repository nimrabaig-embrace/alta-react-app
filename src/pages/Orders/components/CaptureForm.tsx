...
const CaptureForm: React.FC<{}> = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      if (!message) {
        axios
          .post(
            `${ApiConfigURL}/shopOrders/${orderContext?.selectedOrder?.id}/capture`,
            {
              ...values,
            }
          )
  ...
  return (
    <Modal onClose={() => orderContext?.setActionState(null)}>
      <PrimaryHeading>Capture Transaction</PrimaryHeading>
      <hr
        style={{
      ...
      >
        ...
        <Button type="submit">{message ? "Close" : "Capture"}</Button>
      </form>
    </Modal>
  );
};
